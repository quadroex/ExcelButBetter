...
//                // Створюємо потік з тексту
//                using var stream = new MemoryStream(System.Text.Encoding.UTF8.GetBytes(csvContent));

//                // Викликаємо нативний діалог "Зберегти як..."
//                // Користувач сам обирає шлях, ім'я та розширення
//                var fileSaverResult = await FileSaver.Default.SaveAsync("table.csv", stream, CancellationToken.None);

//                if (fileSaverResult.IsSuccessful)
//                {
//                    await DisplayAlert("Успіх", $"Файл збережено: {fileSaverResult.FilePath}", "ОК");
//                }
//            }
//            catch (Exception ex)
//            {
//                await DisplayAlert("Помилка", $"Не вдалося зберегти: {ex.Message}", "ОК");
//            }
//        }
...
//            try
//            {
//                // Дозволяємо і CSV і XLSM (як текст)
//                var fileTypes = new FilePickerFileType(new Dictionary<DevicePlatform, IEnumerable<string>>
//                {
//                    { DevicePlatform.WinUI, new[] { ".csv", ".txt", ".xlsm" } },
//                    { DevicePlatform.Android, new[] { "text/comma-separated-values", "text/plain" } },
...
//                {
//                    PickerTitle = "Оберіть файл таблиці",
//                    FileTypes = fileTypes
//                });
...
//        {
//            GridWidget.ColumnDefinitions.Add(new ColumnDefinition { Width = 100 });
//            var label = new Label { Text = GetColumnName(colIndex), HorizontalOptions = LayoutOptions.Center, VerticalOptions = LayoutOptions.Center, FontAttributes = FontAttributes.Bold, TextColor = Colors.Black };
//            GridWidget.Add(new BoxView { Color = Colors.LightGray }, colIndex + 1, 0);
//            GridWidget.Add(label, colIndex + 1, 0);
//        }
...
//        {
//            GridWidget.RowDefinitions.Add(new RowDefinition { Height = 30 });
//            var label = new Label { Text = (rowIndex + 1).ToString(), HorizontalOptions = LayoutOptions.Center, VerticalOptions = LayoutOptions.Center, FontAttributes = FontAttributes.Bold, TextColor = Colors.Black };
//            GridWidget.Add(new BoxView { Color = Colors.LightGray }, 0, rowIndex + 1);
//            GridWidget.Add(label, 0, rowIndex + 1);
//            for (int col = 0; col < _colCount; col++) CreateCellEntry(col, rowIndex);
//        }
...
//        {
//            string address = GetColumnName(col) + (row + 1);
//            var entry = new Entry { HorizontalTextAlignment = TextAlignment.Start, BackgroundColor = Colors.White, TextColor = Colors.Black };
//            entry.BindingContext = address;
//            entry.Focused += Entry_Focused;
//            entry.Unfocused += Entry_Unfocused;
//            entry.Completed += Entry_Completed;
...
//        private void AddRow_Clicked(object sender, EventArgs e) { AddRow(_rowCount++); UpdateGridDimensions(); }
//        private void AddCol_Clicked(object sender, EventArgs e) { AddColumnHeader(_colCount); for (int row = 0; row < _rowCount; row++) CreateCellEntry(_colCount, row); _colCount++; UpdateGridDimensions(); }
...
//                var toRemove = GridWidget.Children.Where(c => Grid.GetRow((BindableObject)c) == _rowCount + 1).ToList();
//                foreach (var el in toRemove) GridWidget.Children.Remove(el);
//                UpdateGridDimensions(); _gridManager.CleanUpOutOfBounds(); _gridManager.RecalculateAll(); RefreshGrid();
//            }
//        }
...
//                        if (cell.IsText)
//                        {
//                            // Текст показуємо як є
//                            entry.Text = cell.Expression;
//                        }
//                        else
//                        {
//                            // Числа форматуємо через крапку
//                            entry.Text = string.IsNullOrEmpty(cell.Expression) ? "" : cell.Value.ToString(CultureInfo.InvariantCulture);
//                        }
//                        entry.TextColor = Colors.Black;
//                    }
//                }
...
//            string columnName = string.Empty;
//            while (dividend > 0) { int modulo = (dividend - 1) % 26; columnName = Convert.ToChar(65 + modulo) + columnName; dividend = (int)((dividend - modulo) / 26); }
//            return columnName;
//        }

//        private async void HelpButton_Clicked(object? sender, EventArgs e)
//        {
//            await DisplayAlert("Інфо", "Лабораторна робота 1. Варіант 35.\nАвтор: Грицина Антон, К-27\n\nДовідка:\n", "ОК");
//        }
//    }
//}